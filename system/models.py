...
class DepCode(models.Model):
    ...
    #syozoku = models.ForeignKey(syozoku,on_delete=models.PROTECT) # 所属の方に部署コードを追加したため削除
...
class BudCode(models.Model):
    ...
    #syozoku = models.ForeignKey(syozoku,on_delete=models.PROTECT) # 所属の方に予算科目コードを追加したため削除
...
class ProCode(models.Model):
    code = models.CharField(max_length=32,blank=False,null=False,default='Undefined')
    name = models.CharField(max_length=32,blank=False,null=False,default='Undefined')
    #syozoku = models.ManyToManyField(syozoku,related_name="syozoku1") # 所属の方にプロジェクトコードを追加したため削除
    #syozoku = models.ForeignKey(syozoku,on_delete=models.PROTECT)     # 所属の方にプロジェクトコードを追加したため削除
...
class syozoku(models.Model):
    ...
    depcode = models.ManyToManyField(DepCode,related_name="syozoku")  # 部署コード追加（syozokuでいいのか？）
    budcode = models.ManyToManyField(BudCode,related_name="syozoku")  # 予算科目コード追加（syozokuでいいのか？）
    project = models.ManyToManyField(ProCode,related_name="syozoku")  # プロジェクトコード追加（syozokuでいいのか？）
    #project = models.ManyToManyField(ProCode,related_name="syozoku")  # プロジェクトコード追加（syozokuでいいのか？）
...
class Order(models.Model):
    ...
    tanka_zei = models.ForeignKey(tax,on_delete=models.PROTECT,verbose_name='単 価(税込/税抜)',
